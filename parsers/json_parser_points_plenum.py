...
def parse_to_json(lines, file_id):
    ...
    for line in lines:
        ...
        if not title_finished:
            if stripped in ["РОССИЙСКАЯ ФЕДЕРАЦИЯ", "ПРАВИТЕЛЬСТВО РОССИЙСКОЙ ФЕДЕРАЦИИ", "ПРИНЯТ", "ОДОБРЕН", "УТВЕРЖДЕНО", "УТВЕРЖДЕНА", "ПЛЕНУМ ВЕРХОВНОГО СУДА РОССИЙСКОЙ ФЕДЕРАЦИИ"]:
                continue
        ...
        if not in_appendix:
            if "Пункт исключен" in stripped or "Пункты исключены" in stripped:
                continue
